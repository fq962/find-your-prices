 ...
 * cuadrícula.
 */
export function ProductTile({ product, href, mode, density, locale }: ProductTileProps) {
  ...
  return (
    <Link
      href={href}
      className="group flex h-full flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-elevated)] outline-none transition-[border-color,box-shadow,transform] duration-[var(--dur-base)] ease-[var(--ease-out-quart)] hover:-translate-y-0.5 hover:border-[var(--border-strong)] hover:shadow-[var(--shadow-md)] focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)]"
    >
      <div className="relative overflow-hidden bg-[var(--bg-inset)]">
        ...
        {inStock === false && (
          <span className="absolute top-2.5 right-2.5 rounded-full bg-[var(--bg)]/90 px-2 py-0.5 text-[0.6875rem] font-medium text-[var(--text-secondary)] backdrop-blur-sm">
            {availability ?? "Agotado"}
          </span>
        )}
      </div>

      <div className={`flex flex-1 flex-col gap-1 ${isGallery ? "p-5" : "p-3.5"}`}>
        {brand && (
