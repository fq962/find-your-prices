...
| `ACC.` (namespace js), `/_ui/responsive/` | SAP Commerce (Hybris) | `/occ/v2/{baseSiteId}/products/search?query=:name-asc&fields=...` y el arbol en `/occ/v2/{baseSiteId}/catalogs/{catalogo}/Online?fields=FULL`. Ojo: `/rest/v2/` puede responder 404 aunque `/occ/v2/` funcione. |
...
| El html es un interstitial de Akamai (`bm-verify`, `/_sec/verify`) y el sitio es de Inditex (Zara, Bershka, Pull&Bear, Stradivarius...) | SPA propia de Inditex | La misma SPA lee `/{pais}/{idioma}/categories?ajax=true` (arbol) y `/category/{id}/products?ajax=true` (categoria entera, sin paginar). Ojo: Akamai responde 403 a cualquier user agent que no sea de navegador, incluido robots.txt. Ver `zara.ts`. |
...
| Dar por bueno el precio del listado | Precios inflados en parte del catalogo, sin ningun sintoma: el numero existe, es plausible y no falla nada | Compara el precio del listado contra el de la ficha en un articulo rebajado ANTES de escribir el mapeador. En Paper Depot el listado publica el precio de lista tambien cuando hay oferta (164.28 en vez de 89.68): son 1 915 de 5 550 articulos, y la unica fuente del precio real es /ofertas. |
| Medir la latencia del sitio con curl suelto | Un plan de targets de mas: se parte el catalogo en trozos que habrian cabido enteros | `curl` abre conexion nueva en cada llamada; el cliente del proyecto la reusa. En Paper Depot curl marcaba 0,71 s por pagina (417 paginas = 355 s, "no cabe") y el barrido real tardo 0,30 s por pagina: 118 s. Mide con la estrategia, no con curl. |
| Paginar con el orden por relevancia | Faltan articulos sin que falle nada: el barrido devuelve menos de lo que la tienda declara y cada corrida pierde otros distintos | El orden por relevancia se recalcula en cada peticion, asi que un articulo puede saltar de la pagina 7 a la 6 y no salir nunca. Pedi un orden total y estable (`:name-asc`, por codigo, por id). En Office Depot `:relevance` daba 3 909 de 4 056; `:name-asc`, los 4 056 exactos. |
...
| Guardar las fichas "padre" de Shopify Combined Listings | Catalogo duplicado: el perfume agrupado ("... EDC (U)" con 100 y 170 ml) y cada tamaño suelto ("... / 100 ml") como productos distintos, con el mismo id de variante | Mirá si hay tags `combined_listing` y si los ids de variante se repiten entre productos. En Prive eran 893 padres de 3 929 fichas; se descartan con `enrich` devolviendo null, porque la tienda enlaza solo a los hijos y las variantes del padre sin hijo vienen agotadas y sin sku. |
| Tomar el `id` del listado como `external_id` sin mirar que identifica | El mismo articulo entra 2, 5 o 12 veces y cambia de categoria segun el listado | En Zara el `id` es por producto-**color** (y hasta por styling), y cada listado muestra un color distinto del mismo modelo: 24 000 ids para 10 356 modelos. Compará dos listados por el identificador del **modelo** (`seoProductId`, el de la url) antes de elegir; los colores van en `variants`. |
| Un 403 general de la WAF a todo lo que no sea navegador | 403 incluso a `robots.txt` y a `Googlebot`; ninguna otra puerta (sin sitemap de productos) | No es el caso de Walmart (una puerta cerrada, otras abiertas): no hay alternativa. **Se para y se consulta**; en Zara se decidio con el dueño usar `stores.user_agent` (existe para eso) y quedo escrito en la cabecera de la estrategia. La estrategia no disfraza nada por su cuenta y, si el 403 vuelve, lo dice en la bitacora. |
| Dar por catalogo un "VER TODO" | 3 000 articulos exactos: es un tope, no el total | Zara corta VER TODO de hombre en 3 000 componentes y el de Bebe muestra 507 de 1 345 modelos. Recorré las familias (hijas de COLECCIÓN etc.) y dejá los contenedores al final, solo para completar. |
| Sumar raices que son vistas transversales | Un target mas por raiz para nada | En NIÑOS, ACCESORIOS \| ZAPATOS y KDPT Goods aportan 0 y 1 modelo nuevo frente a las cinco raices por edad (medido con un barrido de cobertura). Medí lo que **agrega** cada raiz antes de sumarla al plan. |
| Medir el presupuesto solo con la descarga | La estrategia tarda 200 s suelta y el runner marca `partial` a los 247 s | La ingesta cuenta dentro de los 240 s: ~10 s por cada 1 000 modelos con variantes. Niños de Zara descargaba en 200 s y no cabia; se apagaron los listados genericos (`includeGenericListings: false`, −60 s por 44 modelos). Medí con `POST /targets/{id}/run`, no solo con la estrategia. |
| Resolver la categoria entre targets con un indice de exclusion cuando hay un dato intrinseco | Releer hasta 170 MB por target para saber que es de quien | Antes de copiar el indice de Metromedia, buscá en el propio articulo algo que fije su target: en Zara `sectionName` (WOMAN/MAN/KID) es exacto (medido: 0 modelos con dos secciones, 0 perdidos) y resuelve el solape Mujer/Hombre gratis. Lo que no tiene dato intrinseco (las cinco raices de niños) va junto en **un** target. |
...
| Un ejemplo terminado (sitio PHP propio sin plataforma: html SSR, listado y /ofertas por separado porque el listado publica el precio SIN la rebaja, precios sin impuesto) | [`strategies/paperdepot.ts`](./strategies/paperdepot.ts) |
| Un ejemplo terminado (SAP Commerce / Hybris, API OCC v2 publica; la categoria del articulo se deduce del primer segmento de su url contra el arbol del catalogo, con sondeo para los nombres repetidos) | [`strategies/officedepot.ts`](./strategies/officedepot.ts) |
...
| Una tienda Shopify que **reusa** la fábrica y descarta artículos desde `enrich` (padres de Combined Listings y gift cards), con género, concentración y mililitros sacados de tags y variantes | [`strategies/prive.ts`](./strategies/prive.ts) |
| Un ejemplo terminado (SPA de **Inditex** tras Akamai, API JSON sin paginar; `external_id` por modelo con los colores como `variants`; se recorren las familias del menu y no las 1 084 hojas; un target por seccion con el solape entre secciones resuelto por `sectionName`, intrinseco al articulo; `includeGenericListings` para que Niños quepa en el presupuesto) | [`strategies/zara.ts`](./strategies/zara.ts) |
