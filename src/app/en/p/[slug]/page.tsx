...
export async function generateMetadata({
  ...
  return productMetadata(product, "en", slug);
}
...
export default async function ProductPage({ params }: PageProps) {
  ...
  const path = productPaths(slug).en;
  ...
  return (
    /* La misma ficha en el otro idioma comparte el id, así que su ruta se
       ...
       la portada. */
    <PageLayout locale="en" localePaths={productPaths(slug)}>
      <JsonLd
        data={graph([
          ...
          breadcrumbJsonLd(productBreadcrumbs(product, "en", slug)),
        ])}
      />
