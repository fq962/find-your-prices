...
/**
 * Una categoría dentro de una tienda (en): "Abarrotes en PriceSmart". Ver
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return storePageMetadataFor(store, slug, "en");
}

export default async function StoreCategoryPage({ params }: PageProps) {
  const { store, slug } = await params;
  return <StoreRoute storeSlug={store} categorySlug={slug} locale="en" />;
}
